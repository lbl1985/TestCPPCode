...
std::vector< std::vector<int> > getSubsets(int const *arr, int index){
    ...
    if (index == 0) {
        ...
        std::vector<std::vector<int>> tmp = getSubsets(arr, index - 1);
        subsets = tmp;
        std::for_each(tmp.begin(), tmp.end(), [&subsets, current](std::vector<int> &tmp2){
            tmp2.push_back(current);
            subsets.push_back(tmp2);
        });
    }
    return subsets;
}
